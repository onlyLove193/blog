<?php
	...
	class BlistController extends CommonController{
		...
		public function index(){
			...
			$this->list = $article->field($field)->join("bl_author on bl_author.tid=bl_article.auid")->join("bl_category on bl_category.gid=bl_article.cid")->where(array('bl_author.status'=>1,'bl_category.status'=>1))->limit($page->firstRow.','.$page->listRows)->order("wtime desc")->select();
